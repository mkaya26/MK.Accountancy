...
{
    public partial class Sp_PaymentDocuments : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var sqlProc = @"IF OBJECT_ID('MHSB.Sp_PaymentDocuments', 'P') IS NOT NULL
    DROP PROCEDURE MHSB.Sp_PaymentDocuments
GO
CREATE PROCEDURE [MHSB].[Sp_PaymentDocuments] @DepartmentId uniqueidentifier,
...
GROUP BY
       ...
       ReceiptDetails.MyDocument,
       ReceiptDetails.IsDeleted
HAVING Receipt.DepartmentId = ''' + CONVERT(varchar(MAX), @DepartmentId) + '''
...
END
    ...
    EXEC (@SQL);";
        ...
        {
            var sqlProc = "DROP PROC MHSB.Sp_PaymentDocuments";
            migrationBuilder.Sql(sqlProc);
        }
    }
}
