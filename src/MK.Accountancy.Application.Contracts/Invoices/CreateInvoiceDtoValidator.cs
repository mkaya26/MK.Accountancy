...
namespace MK.Accountancy.Invoices
{
    ...
    {
        public CreateInvoiceDtoValidator(IStringLocalizer<AccountancyResource> localizer)
        {
            RuleFor(x => x.InvoiceType)
                .IsInEnum()
                .WithMessage(localizer[AccountancyDomainErrorCodes.Required, localizer["InvoiceType"]])
                .NotEmpty()
                .WithMessage(localizer[AccountancyDomainErrorCodes.Required, localizer["InvoiceType"]]);
            //
            ...
            RuleFor(x => x.DepartmentId)
                .Must(x => x.HasValue && x.Value != Guid.Empty)
                .WithMessage(localizer[AccountancyDomainErrorCodes.Required, localizer["DepartmentId"]]);
            //
            RuleFor(x => x.TermId)
                .Must(x => x.HasValue && x.Value != Guid.Empty)
